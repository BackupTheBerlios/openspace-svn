...
string filelist::getdefaultcommand(string name, bool resolve = true)
{
	...
	key =
	    conf->
	    readonestring
	    ("/OpenspaceConfig/file_types_speical/all/default");
    }
	...
	if (key != "")
	    res =
		conf->readonestring("/OpenspaceConfig/commands/" + key +
				    "/exec");
	else
	    return "";
...
FXIconList(p, this, ID_ICO,
	   LAYOUT_FILL_X | LAYOUT_FILL_Y | ICONLIST_EXTENDEDSELECT |
	   ICONLIST_COLUMNS)
{
...
//bind keys
    ...
    table->addAccel(MKUINT(KEY_a, CONTROLMASK), this,
		    FXSEL(SEL_COMMAND, filelist::ID_SELECT_ALL));
    table->addAccel(MKUINT(KEY_F5, 0), this,
		    FXSEL(SEL_COMMAND, filelist::ID_REFRESH));
    table->addAccel(MKUINT(KEY_Delete, 0), this,
		    FXSEL(SEL_COMMAND, filelist::ID_REMOVE));
...
*/
	...
	string size =
	    conf->
	    readonestring("/OpenspaceConfig/filelist/local/thumbs/size");
	thumb_size = atoi(size.c_str());
    }

    string style =
	conf->readonestring("/OpenspaceConfig/filelist/" + this->type +
			    "/style");
    if (style == "big icons") {
	this->
	    setListStyle(ICONLIST_EXTENDEDSELECT | ICONLIST_BIG_ICONS |
			 ICONLIST_COLUMNS);
	setFont(captionfont);
    } else if (style == "small icons")
	this->
	    setListStyle(ICONLIST_EXTENDEDSELECT | ICONLIST_MINI_ICONS |
			 ICONLIST_COLUMNS);
    else if (style == "detailed")
	this->
	    setListStyle(ICONLIST_EXTENDEDSELECT | ICONLIST_DETAILED |
			 ICONLIST_COLUMNS);
    ...
    string plugin_path =
	conf->readonestring("/OpenspaceConfig/path") +
	"plugins/filelist/libfilelist" + this->type;
	...
	gg = (filelist_base * (*)(void)) fxdllSymbol(dllhandle,
						     "get_filelist");
	fb = gg();
...
//label=new FXLabel (p,this->path.c_str());
	...
	bottomframe =
	    new FXHorizontalFrame(p, LAYOUT_FILL_X | FRAME_THICK, 0, 0, 0,
				  0, 0, 0, 0, 0);
	textfield =
	    new FXTextField(bottomframe, 30, this,
			    filelist::ID_TEXTFIELD_REG);

	new FXButton(bottomframe, "", osicons[15], this,
		     filelist::ID_TEXTFIELD_REG, BUTTON_TOOLBAR, 0, 0, 0,
		     0, 0, 0, 0, 0);
	new FXButton(bottomframe, "Go", 0, this, filelist::ID_TEXTFIELD_GO,
		     BUTTON_TOOLBAR, 0, 0, 0, 0, 0, 0, 0, 0);
	new FXButton(bottomframe, "Get", 0, this,
		     filelist::ID_TEXTFIELD_GET, BUTTON_TOOLBAR, 0, 0, 0,
		     0, 0, 0, 0, 0);
...
void filelist::init(void)
{
	...
	new FXOption(sortpop, header_vec[indx].c_str(), NULL, this,
		     ID_SORT_CHANGE, JUSTIFY_HZ_APART | ICON_AFTER_TEXT);
    }

    new FXOption(sortpop, "extension", NULL, this, ID_SORT_CHANGE,
		 JUSTIFY_HZ_APART | ICON_AFTER_TEXT);

    sortmenu =
	new FXOptionMenu(bottomframe, sortpop,
			 LAYOUT_TOP | FRAME_RAISED | FRAME_THICK |
			 JUSTIFY_HZ_APART | ICON_AFTER_TEXT);
    sortmenu->create();
    bottomframe->recalc();
    opendir(this->path);
...
void filelist::copymoveremove(string com_name)
{
    ...
    thread_elem *el =
	new thread_elem(fil, com_name, options, srclist,
...
//label->setText(dir.c_str());
	...
	if (os_file.type & FOLDER) {
		...
		readcolor(conf->
			  readonestring
			  ("/OpenspaceConfig/file_types_special/dir/color"));
	    backcolor =
		readcolor2(conf->
			   readonestring
			   ("/OpenspaceConfig/file_types_special/dir/backcolor"));
	    icon = osicons[1];
	    icon2 = osicons[0];
	} else {
		...
		color =
		    readcolor(conf->
			      readonestring
			      ("/OpenspaceConfig/file_types_special/all/color"));
		backcolor =
		    readcolor2(conf->
			       readonestring
			       ("/OpenspaceConfig/file_types_special/all/backcolor"));
	    }
	...
	if (os_file.type & EXECUTABLE && !(os_file.type & FOLDER)) {
		...
		readcolor(conf->
			  readonestring
			  ("/OpenspaceConfig/file_types_special/executable/color"));
	    backcolor =
		readcolor2(conf->
			   readonestring
			   ("/OpenspaceConfig/file_types_special/executable/backcolor"));
	}


	appendItem(new
		   os_ListItem(this, os_file, color, backcolor, icon,
			       icon2));
...
long filelist::openfile(FXObject * sender, FXSelector, void *)
{
	...
	if (res == "INTERNAL")	//internal command, open new window for virtual file system
	{
		...
		string *file =
		    new string(path + SEPARATOR + getItemText(c).text());
		notifyparent->handle(this, FXSEL(SEL_COMMAND, 667),
				     (void *) file);
	    }
	    return 0;
	}
	if (res != "") {
		...
		conf->readonestring("/OpenspaceConfig/commands/" + key +
				    "/options");
	    thread_elem *el =
		new thread_elem(fb, "execute", options, new string(res));
	    start_thread(el);
		...
		conf->
		readonestring
		("/OpenspaceConfig/file_types_special/all/default");
	    if (str != "") {
		string str2 =
		    conf->readonestring("/OpenspaceConfig/commands/" +
					str + "/exec");
		if (str2 != "") {
		    string options =
			conf->readonestring("/OpenspaceConfig/commands/" +
			...
			thread_elem *el =
			    new thread_elem(fb, "execute", options,
					    new string(str2));
			start_thread(el);
		    }
		}
...
long filelist::onPopup(FXObject *, FXSelector, void *ptr)
{
	...
	while (1) {
	    ...
	    shutterItem =
		new FXShutterItem(shutterFrame, res.c_str(), osicons[8],
	    ...
	    if (conflocal.
		openxpath("/OpenspaceConfig/shutter/" + res +
			  "/command") != -1) {
		...
		while (1) {
			...
			for (int i = 0; i < getNumItems(); i++) {
			    if (isItemSelected(i)) {
				sp++;
				os_ListItem *oslistitem =
				    (os_ListItem *) getItem(i);
				getdefaultcommand(oslistitem->osf.name);
				string ext =
				    getfiletype(oslistitem->osf.name);
				string rep = "file_types/" + ext;
				if (oslistitem->osf.type & FOLDER) {
				...
				if (ext != ""
				    && conflocal3.
				    openxpath("/OpenspaceConfig/" + rep +
					      "/commands/command") != -1) {
				    while (1) {

					string res =
					    conflocal3.getnextstring();
					if (res == "")
					    break;
			...
			while (supported_commands.size() > 0) {
			    ...
			    string res2 =
				conflocal2.
				readonestring("/OpenspaceConfig/commands/"
					      + res + "/exec");
			    if (res2 == "INTERNAL")
				comm_s = "IC_";
			    else if (res2 == "PLUGIN")
			    ...
			    new FXButton(shutterItem->getContent(),
					 res.c_str(), 0, this,
					 ID_LAST + command_num,
					 FRAME_RAISED | LAYOUT_FILL_X |
					 LAYOUT_TOP | LAYOUT_LEFT |
					 BUTTON_TOOLBAR, 0, 0, 0, 0, 0, 0,
					 0, 0);
		    ...
		    string res2 =
			conflocal2.
			readonestring("/OpenspaceConfig/commands/" + res +
				      "/exec");
		    if (res2 == "INTERNAL")
			comm_s = "IC_";
		    else if (res2 == "PLUGIN")
		    ...
		    new FXButton(shutterItem->getContent(), res.c_str(), 0,
				 this, ID_LAST + command_num,
				 FRAME_RAISED | LAYOUT_FILL_X | LAYOUT_TOP
				 | LAYOUT_LEFT | BUTTON_TOOLBAR, 0, 0, 0,
				 0, 0, 0, 0, 0);
		    commands_tab[command_num++] = comm_s.c_str();
...
long filelist::file_operation(FXObject * obj, FXSelector sel, void *ptr)
{
	...
	string res =
	    conf->readonestring("/OpenspaceConfig/commands/" + com_name +
				"/exec");
	string options =
	    conf->readonestring("/OpenspaceConfig/commands/" + com_name +
				"/options");
	...
	} else if (com_name == "copy" || com_name == "move"
		   || com_name == "remove") {
	...
	FXASSERT((5, "PLUGIN\n"));
	string res =
	    conf->readonestring("/OpenspaceConfig/plugins/cmddialog/" +
				com_name);
	...
	if (dllhandle) {
		...
		if (isItemSelected(c)) {
		    srclist[ind] =
			path + SEPARATOR + getItemText(c).text();
		    ind++;
		}
	    ...
	    cmddialog *(*gg) (FXWindow *, filelist_base * fb,
			      string * src);
	    gg = (cmddialog *
		  (*)(FXWindow *, filelist_base * fb,
		      string * src)) fxdllSymbol(dllhandle,
						 "get_cmddialog");
...
void *filelist::thread_func(void *data)
{
	...
	fb->init(&filel->vector_name, &filel->vector_type,
		 &filel->vector_width, (*(filel->pt)), conf);
...
long filelist::click(FXObject *, FXSelector, void *ptr)
{
    ...
    string inf =
	"selected: " + ntos(count) + " size: " + numtostring(size);
    if (count == 1) {
	inf = inf + " " + os->osf.name;
    }
