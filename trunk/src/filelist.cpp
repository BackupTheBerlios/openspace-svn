				 ...
				 if(str!="")
				 {
					 ...
					 string fullname="\""+path + SEPARATOR + name+"\"";
						...
						str2.replace(pos,fullname.length(),fullname);
					 	...
					 	thread_elem* el=new thread_elem(fb,"execute",options,new string(str2));
					 	start_thread(el);
						}
					 }
				 }
					...
					string options=conf->readonestring("/OpenspaceConfig/commands/" + com_name + "/options");
						...
						*/
						...
						comm.replace(pos,fullname.length(),fullname);
						 ...
						 thread_elem* el=new thread_elem(fb,"execute",options,new string(comm));
						 start_thread(el);
						 		}
						}
  ...
  q=(const unsigned char*)b->osf.attrib[fl->sort_nr].c_str();   
   	...
   	if(fl->vector_type[fl->sort_nr]==1) //size	
	{
		...
		if(a->osf.size > b->osf.size)
			if(ascend)return 1; else return -1;
		else
			if(ascend)return -1; else return 1;
		...
		while(1)
  		{
			if(strcase)
			{
   				if(*p > *q) if(ascend) return 1; else return -1;
    				if(*p < *q) if(ascend) return -1; else return 1;
			}
			else
			{
   				if(tolower(*p) > tolower(*q)) if(ascend)return 1; else return -1;
   				if(tolower(*p) < tolower(*q)) if(ascend)return -1; else return 1;
			}
